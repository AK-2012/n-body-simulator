class Body:
    def __init__(self, mass: int | float, velocity: list[int | float], position: list[int | float], color: tuple[int, int, int]):
    ...
    def _validate_mass(name, value):
        if not isinstance(value, float | int) or isinstance(value, bool): # must be float or int
